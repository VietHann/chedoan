...
class NutritionBloc extends Bloc<NutritionEvent, NutritionState> {
  ...
  Future<void> _onLoadDailyNutrition(
    ...
    try {
      ...
      if (user == null) {
        emit(NutritionError(message: 'User not found'));
        return;
      }
      ...
      emit(NutritionError(message: 'Failed to load nutrition data: $e'));
    }
  }
  ...
  Future<void> _onAddMealEntry(
    ...
    try {
      ...
      emit(NutritionError(message: 'Failed to add meal: $e'));
  ...
  Future<void> _onDeleteMealEntry(
    ...
    try {
      ...
      emit(NutritionError(message: 'Failed to delete meal: $e'));
      // Restore previous state
      if (currentState is DailyNutritionLoaded) {
        emit(currentState);
      }
  ...
  Future<void> _onAddWaterIntake(
    ...
    try {
      ...
      emit(NutritionError(message: 'Failed to add water intake: $e'));
      // Restore previous state
      if (currentState is DailyNutritionLoaded) {
        emit(currentState);
      }
  ...
  Future<void> _onLoadWeeklyStats(
    ...
    try {
      ...
      emit(NutritionError(message: 'Failed to load weekly stats: $e'));
    }
  }
}