...
class _FoodSearchScreenState extends State<FoodSearchScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadInitialData() async {
    ...
    try {
      final foodRepository = Provider.of<FoodRepository>(context, listen: false);
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Error loading food data: $e')),
      );
    }
  }
  ...
  Future<void> _searchFood(String query) async {
    ...
    try {
      ...
      List<FoodItem> localResults = await foodRepository.searchFoodLocal(query);
      
      // Also search API
      List<FoodItem> apiResults = await foodRepository.searchFoodAPI(query);
      
      // Combine results, removing duplicates (would need a proper way to identify duplicates)
      final combinedResults = [...localResults];
      
      // Add API results that aren't already in local results
      // In a real app, you'd need a better way to identify duplicates
      for (var apiItem in apiResults) {
        if (!localResults.any((item) => item.name.toLowerCase() == apiItem.name.toLowerCase())) {
          combinedResults.add(apiItem);
        }
      }
    ...
    try {
      ...
      if (foodItem.id != null) {
        ...
        setState(() {
          ...
          final int searchIndex = _searchResults.indexWhere((item) => item.id == foodItem.id);
          if (searchIndex >= 0) {
          ...
          // Update the item in recent items
          final int recentIndex = _recentItems.indexWhere((item) => item.id == foodItem.id);
          if (recentIndex >= 0) {
            _recentItems[recentIndex] = _recentItems[recentIndex].copyWith(
              isFavorite: !_recentItems[recentIndex].isFavorite,
            );
          }
        });
      } else {
        // If the food item doesn't have an ID, it hasn't been saved to the database yet
        final savedId = await foodRepository.addFoodItem(foodItem.copyWith(
          isFavorite: true,
        ));
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const Text(
              'Type in the search box to find foods',
              style: TextStyle(
                color: AppTheme.secondaryTextColor,
              ),
            ),
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            const Text(
              'Foods you log will appear here for easy access',
              style: TextStyle(
                color: AppTheme.secondaryTextColor,
              ),
            ),
